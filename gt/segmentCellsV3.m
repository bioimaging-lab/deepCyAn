function mask = segmentCellsV3(If, Ib)
...
%Generate an initial mask using the adaptive thresholding algorithm on the
%brightfield image
T = adaptthresh(Ib, 0.8, 'NeighborhoodSize', 41);
mask = imbinarize(Ib, T);
...
imshowpair(Ib, bwperim(mask))
keyboard
%% Refine using the fluorescence channel?
